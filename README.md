...
Замороженный 250-строчный демо-набор станет доступен после одноразового запуска `scripts/freeze_demo.py` с ключом Anthropic и проверки `scripts/score_demo.py`. До появления честных модельных результатов кнопка в интерфейсе отключена и помечена «Скоро».
