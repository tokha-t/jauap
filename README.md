...
Откройте адрес, показанный Streamlit, и нажмите **«Загрузить демо-набор (250 обращений)»**. В офлайн-режиме приложение читает готовый `data/demo_results.json`; ключ Anthropic не нужен, сетевые вызовы провайдера и картографических тайлов не выполняются.

Для живой классификации собственного синтетического текста:

```bash
export ANTHROPIC_API_KEY="..."
export JAUAP_MODEL="claude-sonnet-4-5"
streamlit run app.py
```

Все обращения при ошибке провайдера остаются в очереди: применяется резервная классификация с уверенностью `0.3` и обязательной ручной проверкой. Ответы Anthropic кешируются в `data/.llm_cache/` по SHA-256 от системного промпта, пользовательского текста и модели.
...
Офлайн-карта использует встроенную схему границ, озера Копа и рек Шағалалы и Кылшакты. Это не официальная картографическая подложка.
...
jauap/llm.py              единственная точка вызова Anthropic и кеш
data/demo_corpus.json     250 исходных синтетических обращений
data/demo_results.json    полностью предвычисленный офлайн-результат
```
