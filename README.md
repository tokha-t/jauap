...
Замороженный 250-строчный демо-набор станет доступен после запуска возобновляемого `scripts/freeze_demo.py` с ключом выбранного провайдера и проверки `scripts/score_demo.py`. Скрипт сохраняет контрольную точку после каждого обращения и при повторном запуске использует уже записанный кеш. До появления честных модельных результатов кнопка в интерфейсе отключена и помечена «Скоро».

Для живой классификации собственного синтетического текста:

```bash
export JAUAP_PROVIDER="gemini"       # gemini | groq | anthropic
export GOOGLE_API_KEY="..."          # ключ только выбранного провайдера
# export JAUAP_MODEL="gemini-3.6-flash"  # необязательное переопределение
streamlit run app.py
```

Ввод API-ключа в самом интерфейсе помечен «Скоро». Без ключа живой ввод не падает: он переходит на правила, ставит уверенность `0.3` и требует ручную проверку.

Все обращения при ошибке провайдера остаются в очереди: применяется резервная классификация с уверенностью `0.3` и обязательной ручной проверкой. Ответы Gemini, Groq или Anthropic кешируются в `data/.llm_cache/`; ключ кеша — SHA-256 от провайдера, модели, системного промпта и пользовательского текста. SDK провайдеров импортируются только внутри `jauap/llm.py`.
...
Все демонстрационные обращения полностью синтетические. Система не обрабатывала ни одного реального обращения гражданина. Пока `data/demo_results.json` не создан, интерфейс прямо сообщает об этом; для текущей заморозки выбран Gemini `gemini-3.6-flash`. После завершения фактические `provider`, `model` и `generated_at` записываются в файл результатов и показываются в боковой панели.
...
2. На вкладке **«Карта»** переключить 249 отдельных отметок на 124 кластерные. Двадцать формулировок о порыве трубы во дворе дома 14 по улице Абая образуют ровно один кластер `CL-001`.
...
jauap/llm.py              единая граница Gemini / Groq / Anthropic и кеш
