...
В репозитории находится замороженный 250-строчный демо-набор, созданный Gemini `gemini-3.1-flash-lite` через реальный путь `classify_text()` и проверенный `scripts/score_demo.py`. Возобновляемый `scripts/freeze_demo.py` сохраняет контрольную точку после каждого обращения и при повторном запуске использует уже записанный кеш.
...
Все демонстрационные обращения полностью синтетические. Система не обрабатывала ни одного реального обращения гражданина. `data/demo_results.json` создан Gemini `gemini-3.1-flash-lite` 26 августа 2026 года; фактические `provider`, `model` и `generated_at` записаны в файле результатов и показаны в боковой панели. Измеренная точность `appeal_type` на изолированной разметке — **43,2%**, а не рекламная оценка; `topic` — **94,4%**, `settlement` — **100,0%**.
