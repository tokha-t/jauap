...
Все демонстрационные обращения полностью синтетические. Система не обрабатывала ни одного реального обращения гражданина. `data/demo_results.json` создан Gemini `gemini-3.1-flash-lite` 26 августа 2026 года; фактические `provider`, `model` и `generated_at` записаны в файле результатов и показаны в боковой панели. Измеренная точность `appeal_type` на изолированной разметке — **90,4%**, а не рекламная оценка; `topic` — **95,2%**, `settlement` — **100,0%**.
