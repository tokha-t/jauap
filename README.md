...
Все демонстрационные обращения полностью синтетические. Система не обрабатывала ни одного реального обращения гражданина. Пока `data/demo_results.json` не создан, интерфейс прямо сообщает об этом; для текущей заморозки выбран стабильный высокопоточный Gemini `gemini-3.1-flash-lite`. После завершения фактические `provider`, `model` и `generated_at` записываются в файл результатов и показываются в боковой панели.
